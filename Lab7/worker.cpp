...
std::list<worker> worker::getInitialWorkers(const std::string& fileName) {
    std::ifstream file(fileName);

    if (!file.is_open()) return {};

    int count = readInt(file);
    std::list<worker> result(count);

    for (worker& it : result) {
        file >> it;
    }
...
void worker::saveWorkers(const std::list<worker>& workers, const std::string& fileName) {
